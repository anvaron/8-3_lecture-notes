 ...
 * 2
 *
 * Update the code below so that it uses .forEach() instead of a for loop. 
 * Inside of printAllComics() you will have to write another, anonymous function.
 */
...
*/
function printAllComics(comics) {
  comics.forEach(function (comic, index, array) {
    console.log(`${index} out of ${array.length} ${comic.title} by ${comic.author}`)
  });
}
